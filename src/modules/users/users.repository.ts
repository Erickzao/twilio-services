...
export class UsersRepository {
  ...
  async create(input: CreateUserInput): Promise<User> {
    ...
    const userQuery = `
      ...
      VALUES (?, ?, ?, ?, ?)
    `;
    await this.client.execute(userQuery, [id, input.email, input.name, now, now], {
      prepare: true,
    });
  ...
  async update(id: string, input: UpdateUserInput): Promise<User | null> {
    ...
    if (input.email !== undefined && input.email !== existing.email) {
      ...
      // Cria novo lookup
      const insertLookupQuery = 'INSERT INTO users_by_email (email, user_id) VALUES (?, ?)';
      await this.client.execute(insertLookupQuery, [input.email, types.Uuid.fromString(id)], {
        prepare: true,
      });
    }
